        ...
        <div className="p-6">
          <div className="max-w-6xl mx-auto">
            <div className="bg-amber-600/20 border border-amber-500/30 rounded-lg p-6 text-center">
              <h3 className="text-xl font-semibold text-amber-400 mb-2">No Threat Data Available</h3>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <div className="bg-gradient-to-r from-red-600/20 to-red-500/20 border border-red-500/30 rounded-lg p-4">
              <p className="text-red-300">
                ...
                These techniques have been identified as applicable to your organization based on threat actor analysis and organizational profile.
              </p>
            </div>
          </div>
          ...
          <div className="mb-6 flex flex-col sm:flex-row gap-4">
            ...
            <div className="text-sm text-secondary-400 flex items-center">
              Showing {startIndex + 1}-{Math.min(startIndex + itemsPerPage, filteredThreats.length)} of {filteredThreats.length} threats
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-secondary-900/50">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-medium text-secondary-400 uppercase tracking-wider">Select</th>
                    <th className="px-6 py-4 text-left text-xs font-medium text-secondary-400 uppercase tracking-wider">Technique</th>
                    <th className="px-6 py-4 text-left text-xs font-medium text-secondary-400 uppercase tracking-wider">Tactic</th>
                    <th className="px-6 py-4 text-left text-xs font-medium text-secondary-400 uppercase tracking-wider">Applicability</th>
                    <th className="px-6 py-4 text-left text-xs font-medium text-secondary-400 uppercase tracking-wider">Detection Methods</th>
                  </tr>
                </thead>
                  ...
                  {paginatedThreats.map((threat, index) => (
                    <tr 
                      ...
                      className={`hover:bg-secondary-700/30 transition-colors ${
                        selectedThreats.includes(threat.technique_id) ? 'bg-primary-600/10' : ''
                      }`}
                    >
                      <td className="px-6 py-4">
                        <button
                          onClick={() => handleThreatToggle(threat.technique_id)}
                      ...
                      <td className="px-6 py-4">
                        <div>
                          <div className="text-white font-medium">{threat.technique_name}</div>
                          <div className="text-sm text-secondary-400">{threat.technique_id}</div>
                          <div className="text-sm text-secondary-500 mt-1">{threat.description}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1 rounded-full text-sm font-medium ${getTacticColor(threat.tactic)}`}>
                          {threat.tactic}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-2">
                          <span className={`font-semibold ${getApplicabilityColor(threat.applicability_score)}`}>
                            {Math.round(threat.applicability_score * 100)}%
                          </span>
                          <div className="w-16 bg-secondary-700 rounded-full h-2">
                            <div 
                              className={`h-2 rounded-full ${
                                threat.applicability_score >= 0.8 ? 'bg-red-400' :
                                threat.applicability_score >= 0.6 ? 'bg-amber-400' :
                                threat.applicability_score >= 0.4 ? 'bg-yellow-400' : 'bg-green-400'
                              }`}
                              style={{ width: `${threat.applicability_score * 100}%` }}
                            ></div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="space-y-1">
                          {threat.detection_methods && threat.detection_methods.length > 0 ? (
                            threat.detection_methods.slice(0, 2).map((method, idx) => (
                              <div key={idx} className="text-sm text-secondary-300">
                                • {method}
                              </div>
                            ))
                          ) : (
                            <div className="text-sm text-secondary-500">No detection methods specified</div>
                          )}
                          {threat.detection_methods && threat.detection_methods.length > 2 && (
          ...
          {selectedThreats.length > 0 && (
            <div className="mt-8 bg-secondary-800/50 rounded-xl border border-secondary-700/50 p-6">
              ...
              <p className="text-secondary-400 text-sm">
                These selected threats will be prioritized for detection and mitigation strategies.
              </p>
            </div>
          )}
