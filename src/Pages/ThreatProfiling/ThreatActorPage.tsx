        ...
        <div className="p-6">
          <div className="max-w-6xl mx-auto">
            <div className="bg-amber-600/20 border border-amber-500/30 rounded-lg p-6 text-center">
              <h3 className="text-xl font-semibold text-amber-400 mb-2">No Threat Actor Data Available</h3>
          ...
          <div className="mb-8">
            ...
            <div className="bg-gradient-to-r from-amber-600/20 to-amber-500/20 border border-amber-500/30 rounded-lg p-4">
              <p className="text-amber-300">
                ...
                These threat actors have been identified as potentially relevant to your organization based on your industry, geography, and digital footprint.
              </p>
            </div>
          </div>
              ...
              >
                <div className="flex items-start justify-between mb-6">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-6 text-sm">
                        <div className="flex items-center space-x-2">
                          <span className="text-secondary-400">Relevance Score:</span>
                          <span className={`font-semibold ${getRelevanceColor(actor.relevance_score)}`}>
                            {actor.relevance_score}%
                          </span>
                        </div>
                        <div className="flex items-center space-x-2">
                          <span className="text-secondary-400">Sophistication:</span>
                          <span className={`font-semibold ${getSophisticationColor(actor.sophistication)}`}>
                  ...
                  <Button
                    ...
                    variant={selectedActors.includes(actor.id) ? "primary" : "outline"}
                    className="px-4 py-2"
                  >
                    ...
                    <div className="space-y-2">
                      {actor.motivation && actor.motivation.length > 0 ? (
                        actor.motivation.map((motivation, idx) => (
                          <span key={idx} className="inline-block px-3 py-1 bg-amber-600/20 text-amber-300 text-sm rounded-full mr-2 mb-2">
                            {motivation}
                          </span>
                        ))
                      ) : (
                        <p className="text-secondary-400 text-sm">No specific motivations identified</p>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="space-y-2">
                      {actor.capabilities && actor.capabilities.length > 0 ? (
                        actor.capabilities.map((capability, idx) => (
                          <span key={idx} className="inline-block px-3 py-1 bg-red-600/20 text-red-300 text-sm rounded-full mr-2 mb-2">
                            {capability}
                          </span>
                        ))
                      ) : (
                        <p className="text-secondary-400 text-sm">No specific capabilities identified</p>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="space-y-2">
                      {actor.techniques && actor.techniques.length > 0 ? (
                        <div className="space-y-1">
                          {actor.techniques.slice(0, 3).map((technique, idx) => (
                            <div key={idx} className="text-secondary-300 text-sm">
                              • {technique}
                            </div>
                          ))}
                        ...
                        <p className="text-secondary-400 text-sm">No specific techniques identified</p>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div className="flex justify-between items-center mt-8 pt-6 border-t border-secondary-700">
            ...
            <div className="text-center">
              <p className="text-sm text-secondary-400">
                {selectedActors.length} of {threatActors.length} threat actors selected
              </p>
            </div>
            <Button 
