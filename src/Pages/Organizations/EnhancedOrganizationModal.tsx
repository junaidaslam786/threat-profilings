...
import type { CreateOrgDto, LeCreateOrgDto, UpdateOrgDto, ClientDataDto } from "../../Redux/slices/organizationsSlice";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const countriesArray = fields.countriesOfOperation
        ? fields.countriesOfOperation.split(",").map(country => country.trim()).filter(country => country)
        : [];
      ...
      if (isEditing && editingOrg) {
        ...
        const updateData: UpdateOrgDto = {
          ...
          countriesOfOperation: countriesArray.length > 0 ? countriesArray : undefined,
      ...
      if (typeof err === 'object' && err !== null && 'data' in err) {
        ...
        setError(errorData.data?.message || `Failed to ${isEditing ? 'update' : 'create'} organization`);
      } else {
