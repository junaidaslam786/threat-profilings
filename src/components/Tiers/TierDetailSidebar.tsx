...
const TierDetailSidebar: React.FC<TierDetailSidebarProps> = ({ tier, isOpen, onClose }) => {
  if (!tier) return null;
  ...
  return (
    <Sidebar isOpen={isOpen} onClose={onClose} title={`${tier.name} Details`}>
      <div className="space-y-6">
        ...
        <div className="bg-secondary-800 rounded-xl p-6 border border-secondary-700/50">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-secondary-700/50 p-4 rounded-lg">
              <div className="text-2xl font-bold text-primary-400">{tier.max_edits}</div>
              <div className="text-sm text-secondary-400">Max Edits</div>
            </div>
            <div className="bg-secondary-700/50 p-4 rounded-lg">
              <div className="text-2xl font-bold text-primary-400">{tier.max_apps}</div>
              <div className="text-sm text-secondary-400">Max Apps</div>
            </div>
            <div className="bg-secondary-700/50 p-4 rounded-lg">
              <div className="text-2xl font-bold text-primary-400">{tier.run_quota}</div>
              <div className="text-sm text-secondary-400">Run Quota</div>
            </div>
            <div className="bg-secondary-700/50 p-4 rounded-lg">
              <div className="text-2xl font-bold text-primary-400">{features.max_users || 'Unlimited'}</div>
        ...
        <div className="bg-secondary-800 rounded-xl p-6 border border-secondary-700/50">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gradient-to-br from-green-500/20 to-green-600/20 p-4 rounded-lg border border-green-500/30">
              <div className="text-2xl font-bold text-green-400">${tier.price_monthly}</div>
            ...
            <div className="bg-gradient-to-br from-blue-500/20 to-blue-600/20 p-4 rounded-lg border border-blue-500/30">
              <div className="text-2xl font-bold text-blue-400">${tier.price_onetime_registration}</div>
              <div className="text-sm text-blue-300">Registration</div>
            </div>
          </div>
          {features.discount_percent && features.discount_percent > 0 && (
            <div className="mt-4 p-3 bg-yellow-500/20 rounded-lg border border-yellow-500/30">
              <div className="text-yellow-400 font-semibold">{features.discount_percent}% Discount Available</div>
              {features.promotion_code && (
                <div className="text-sm text-yellow-300">Code: {features.promotion_code}</div>
              )}
            </div>
          )}
        </div>
        ...
        {/* Allowed Features */}
        <div className="bg-secondary-800 rounded-xl p-6 border border-secondary-700/50">
          <h3 className="text-lg font-semibold text-primary-300 mb-4">Allowed Tabs</h3>
        ...
        {(features.compliance_frameworks?.length || features.storage_limit_gb) && (
          <div className="bg-secondary-800 rounded-xl p-6 border border-secondary-700/50">
            <h3 className="text-lg font-semibold text-primary-300 mb-4">Additional Information</h3>
            
            {features.storage_limit_gb && (
              <div className="mb-4">
                <label className="text-sm text-secondary-400">Storage Limit</label>
                <div className="text-white font-medium">{features.storage_limit_gb} GB</div>
              </div>
            )}

            {features.compliance_frameworks && features.compliance_frameworks.length > 0 && (
              <div>
                <label className="text-sm text-secondary-400 mb-2 block">Compliance Frameworks</label>
                <div className="flex flex-wrap gap-2">
